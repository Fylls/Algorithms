...
var number = function (busStops) {
  ...
  for (let i = 0; i < busStops.length; i++) {
    let peopleEntering = busStops[i][0];
    let peopleLeaving = busStops[i][1];

    // peopleOnTheBus = peopleOnTheBus + peopleEntering - peopleLeaving;
    peopleOnTheBus += peopleEntering - peopleLeaving;
  }
...
// contracted
var number = function (busStops) {
  var peopleOnTheBus = 0;
  for (var i = 0; i < busStops.length; i++) {
    peopleOnTheBus += busStops[i][0]; // entering
    peopleOnTheBus -= busStops[i][1]; // leaving
  }
  return peopleOnTheBus;
};
