...
//helper function
function capitalizeByIndex(word, index) {
  let upper = word.charAt(index);
  str = `${word.slice(0, index)}${upper.toUpperCase()}${word.slice(index + 1)}`;
  return str;
}
...
function wave(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    wave.push(capitalizeByIndex(str, i));
  }
  return wave;
}
